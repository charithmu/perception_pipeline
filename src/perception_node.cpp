...
int main(int argc, char *argv[])
{
  ...
  auto downsample_filter = nh.param<std::string>("downsample_filter", "none");
  auto resize_filter = nh.param<std::string>("resize_filter", "none");
  ...
  if (downsample_filter == "octree")
  {
    ...
    voxel_leaf_size = nh.param<float>("voxel_leaf_size", 0.1);
  }
  ...
  if (resize_filter == "passthrough")
  {
    ...
    x_filter_min = nh.param<float>("x_filter_min", -2.5);
    x_filter_max = nh.param<float>("x_filter_max", 2.5);
    y_filter_min = nh.param<float>("y_filter_min", -2.5);
    y_filter_max = nh.param<float>("y_filter_max", 2.5);
    z_filter_min = nh.param<float>("z_filter_min", -2.5);
    z_filter_max = nh.param<float>("z_filter_max", 2.5);
  }
  else
  {
  ...
  save_pcd = nh.param<bool>("save_pcd", false);
  save_interval = nh.param<float>("save_interval", 2.0);
  auto save_path = nh.param<std::string>("save_path", "output");
